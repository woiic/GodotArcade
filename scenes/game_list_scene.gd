...
func ShowGameInfoImg(folder_direction):
	...
	#Debug.log(temp_size)
	print(temp_size)
	if (temp_size.x < 576 and temp_size.y < 320):
		game_img.scale = Vector2(1, 1) #Vector2(240 / temp_size.x, 320 / temp_size.y)
	else:
		game_img.scale = Vector2(576.0 / temp_size.x, 320.0 / temp_size.y) #Vector2(240 / temp_size.x, 320 / temp_size.y)
